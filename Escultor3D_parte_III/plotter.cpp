...
void Plotter::paintEvent(QPaintEvent *event)
{
    ...
    pen.setColor(QColor(QColorConstants::Svg::purple));
    painter.setPen(pen);
    painter.setBrush(brush);
    QRect retangle(0,0,width(),height());
    painter.drawRoundedRect(retangle, 20.0, 20.0);

    if(num_linhas !=0 && num_colunas!=0 && num_planos !=0){
        h_altura = (double)height()/num_linhas;
        h_largura = (double)width()/num_colunas;

        pen.setColor(QColor(186, 88, 207, 100));
...
void Plotter::openFile(QString path){}
